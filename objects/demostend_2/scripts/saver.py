...
class Saver():
    """
    Класс для реализции сохранения сигнала и метки классификации
    Реализовано сохранение во вложенном формате (nested)
    """
    def __init__(self, save_path, zone_num, max_files_count):
        """
        Инициализирует Saver
        Args:
            save_path (str): путь для сохранения тревог
            zone_num (int): номер канала охраняемой зоны 
            max_files_count (int): максимальное кол-во тревог
            в файле hdf5
        """
    ...
    def save_alarm(self, signal, probabilities):
        """
        Сохранение сырого сигнала, вероятностей классификатора
        Args:
            signal (np.ndarray): сигнал который привел к возникновению тревоги
            probabilities (_type_): dict в формате {имя_класса: вероятность_класса}
        """
        now = datetime.now()
        ...
        if not os.path.exists(self.filepath):
            ...
            # отметка о том, что это директория, содержащая датасеты
